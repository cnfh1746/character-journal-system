...
#### v1.2.3 (2025-11-12)
**新功能 & 修复**：
- ✨ **新增API重试机制**：提高API调用的可靠性
  - 自动重试：429、500、502、503、504状态码自动重试
  - 重试策略：3次重试，延迟分别为5秒、10秒、20秒
  - 网络错误：fetch/network/timeout错误也会自动重试
  - 用户提示：重试时显示倒计时toastr提示
  - 成功反馈：重试成功后会显示"经过X次重试"
  
- 🐛 **修复世界书绑定问题**（待实现）：
  - 问题描述：角色切换时世界书可能未正确绑定到聊天
  - 计划方案：增强`bindWorldbookToChat()`函数的可靠性
  - 影响范围：`character_main`模式下的角色切换
...
**用户体验改进**：
- 🔄 重试过程透明：显示"API调用失败(429)，5秒后重试(1/3)..."
- ⏱️ 延迟递增：避免频繁重试导致更多限流
- ✅ 成功提示：经过重试成功后会特别提示
- ❌ 失败提示：3次重试都失败后清晰说明
...
**影响范围**：
- ✅ 自定义API：完整支持重试机制
- ✅ ST默认API：也支持重试（generateRaw错误时）
- 🎯 重试逻辑：适用于所有AI调用（识别角色、生成日志、精炼日志）

**世界书绑定问题分析**：
- 📌 问题：`character_main`模式切换角色后，新世界书可能未绑定到聊天
- 🔍 原因：`bindWorldbookToChat()`依赖ST内部API，可能存在兼容性问题
- 💡 临时方案：用户需手动在聊天设置中选择世界书
- 🛠️ 计划方案：增强绑定逻辑，增加验证和重试机制
